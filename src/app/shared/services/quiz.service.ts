...
export class QuizService {
  quizContent: any[] = [];
  quizContent$ = new BehaviorSubject<any[]>(this.quizContent);
  ...
  resetQuiz() {
    ...
    this.quizContent$.next(this.quizContent);
  }

  loadQuestionsForCategory(categoryId: number) {
    this.resetQuiz();
    this.http.get<any>(`http://localhost:3000/categories/${categoryId}`).subscribe(category => {
      const questions = category?.questions || [];
      for (const q of questions) {
        const questionId = q.id;
        const questionLabel = q.questionLabel || q.question || '';
        this.http.get<any>(`http://localhost:3000/answers?questionId=${questionId}`).subscribe(answers => {
          this.quizContent.push({ id: questionId, question: questionLabel, answers });
          this.quizContent$.next([...this.quizContent]);
